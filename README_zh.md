...
**Windows 一键启动**：双击 `start.bat` 即可（自动切到脚本所在目录、设置 UTF-8 代码页、找不到 Node.js 会给出提示并停住窗口）。
...
启动后 cmd 里是一个带序号的菜单，**不会自动刷屏**——需要时按 `1` 查看当前套餐可用模型、按 `3` 查看套餐额度。想只要纯日志（不要控制台）加 `--no-tui`：
...
| `3` | 查看当前套餐额度：5 小时 / 每周 / 每月 三个用量窗口（进度条 + 剩余 + 重置倒计时）、额度池与本期已用、**累计用量（tokens，按 M 显示）**、计费周期、**刷新时间**。30 秒内重复按直接复用缓存；想立刻刷新用 `[8]` |
...
三个用量窗口与官方 CLI 的 `/usage` 面板一致：

- **5小时 / 每周**来自 `billing/credits` 响应的 `windowLimits`（注意它是**顶层字段**，与 `credits` 平级；窗口字段是 `used` / `cap`，`resetAt` 是 epoch 毫秒）。进度条按 `used/cap` 计算，颜色随占用率变化（<70% 绿、≥70% 黄、≥90% 红），窗口超限会标红提示。
- **每月**在服务端 `windowLimits` 里通常没有，用套餐周期额度自己算一条：`已用 / 额度池`，并显示续期时间。
- 每个窗口都显示**距重置的剩余时长 + 重置时刻**（当天只显示时间，跨天带日期），最后一行是**刷新时间**（绝对时刻）与本次拉取耗时。
...
- **累计用量** = 本计费周期内的 token 总量（按官方口径以 M 显示，如 `233.4M`），附输入/输出拆分、请求次数与单次均价。

> ⚠️ `usage/summary` 的 `since` 参数实测被服务端忽略（传 2020 年也返回同样数字，`periodBasis` 恒为 `billing-period`），所以这是**本计费周期**的累计，不是全部历史——界面上如实标注「统计自本计费周期起点」。
>
...
- **每次操作输出完会自动重新打印一遍菜单**，列表/日志滚过屏幕后不用往上翻就能接着选。启动时只摆菜单，不会自动拉模型刷屏。
